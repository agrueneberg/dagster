...
class AirflowDefinitionsData:
    ...
    @cached_property
    def mapped_asset_keys_per_task_handle(self) -> Mapping[TaskHandle, AbstractSet[AssetKey]]:
    ...
    @cached_property
    def mapped_asset_keys_per_dag_handle(self) -> Mapping[DagHandle, AbstractSet[AssetKey]]:
        asset_keys_per_handle = defaultdict(set)
        for spec in self.mapped_defs.get_all_asset_specs():
    ...
    @cached_property
    def peered_dag_asset_keys_by_handle(self) -> Mapping[DagHandle, AbstractSet[AssetKey]]:
        asset_keys_per_handle = defaultdict(set)
        for spec in self.mapped_defs.get_all_asset_specs():
    ...
    def asset_keys_in_task(self, dag_id: str, task_id: str) -> AbstractSet[AssetKey]:
        return self.mapped_asset_keys_per_task_handle[TaskHandle(dag_id=dag_id, task_id=task_id)]
