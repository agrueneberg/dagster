...
class RemoteAssetNode(BaseAssetNode, ABC):
    ...
    def key(self) -> AssetKey:
        return self.resolve_to_singular_repo_scoped_node().asset_node_snap.asset_key
...
@record
class RemoteGlobalAssetGraph(RemoteAssetGraph[RemoteGlobalAssetNode]):
    remote_asset_nodes_by_key: Mapping[AssetKey, RemoteGlobalAssetNode]
    remote_asset_check_nodes_by_key: Mapping[AssetCheckKey, RemoteAssetCheckNode]

    @property
    def _asset_nodes_by_key(self) -> Mapping[AssetKey, RemoteGlobalAssetNode]:
        return self.remote_asset_nodes_by_key
    ...
    def build(cls, workspace: WorkspaceSnapshot):
        ...
        asset_checks_by_key: Dict[AssetCheckKey, RemoteAssetCheckNode] = {}
        for repo in repos:
            for asset_node in repo.asset_graph.asset_nodes:
        ...
        for key, asset_infos in asset_infos_by_key.items():
            _warn_on_duplicate_nodes(asset_infos)
            asset_nodes_by_key[key] = RemoteGlobalAssetNode(
...
def _warn_on_duplicate_nodes(
    infos: Sequence[RepositoryScopedAssetInfo],
) -> None:
    ...
    for info in infos:
        ...
        handle = info.asset_node.repository_handle
        if snap.is_source and snap.is_observable:
            observable_node_pairs.append((handle, snap))
        elif snap.is_source:
            unexecutable_node_pairs.append((handle, snap))
        else:
            materializable_node_pairs.append((handle, snap))
